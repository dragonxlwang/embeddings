...
#define EXP(x) ((fabs((x)) < s3eSquashTableRange) ? \
         s3eExpTable[(int) (((real) (x) / s3eSquashTableRange / 2.0 + 0.5) \
                              * s3eSquashTableSize) ] : 0)
#define SIGM(x) (((x) <= -s3eSquashTableRange) ? 0 : \
                (((x) >=  s3eSquashTableRange) ? 1 : \
        s3eSigmTable[(int) (((real) (x) / s3eSquashTableRange / 2.0 + 0.5) \
                              * s3eSquashTableSize) ]))
#define TANH(x) (((x) <= -s3eSquashTableRange) ? -1 : \
                (((x) >=  s3eSquashTableRange) ? 1  : \
        s3eTanhTable[(int) (((real) (x) / s3eSquashTableRange / 2.0 + 0.5) \
                              * s3eSquashTableSize) ]))
#define MAX(arr, l) ({ __typeof__(arr[0]) tmp_max_x = arr[0]; int tmp_max_i; \
                      for(tmp_max_i = 1; tmp_max_i< l; tmp_max_i++) \
                        tmp_max_x = (tmp_max_x < arr[tmp_max_i]) ? \
                          arr[tmp_max_i] : tmp_max_x; tmp_max_x;})
#define ABS(x) ((x) < 0 ? -(x) : (x))
#define ABSMAX(arr, l) ({ __typeof__(arr[0]) tmp_max_x = ABS(arr[0]); \
                          int tmp_max_i; \
                      for(tmp_max_i = 1; tmp_max_i< l; tmp_max_i++) \
                        tmp_max_x = (tmp_max_x < ABS(arr[tmp_max_i])) ? \
                          ABS(arr[tmp_max_i]) : tmp_max_x; tmp_max_x;})
#define ABSSUM(arr, l) ({ __typeof__(arr[0]) tmp_ss_x = 0; int tmp_ss_i; \
                      for(tmp_ss_i = 0; tmp_ss_i < l; tmp_ss_i++) \
                        tmp_ss_x += ABS(arr[tmp_ss_i]); tmp_ss_x;})
#define CADD(arr, l, x) ({ int tmp_cadd_i; \
                        for(tmp_cadd_i = 0; tmp_cadd_i < l; tmp_cadd_i++) \
                          arr[tmp_cadd_i] += x; })
#define SUM(arr, l) ({ __typeof__(arr[0]) tmp_sum_s = 0; int tmp_sum_i; \
                       for(tmp_sum_i = 0; tmp_sum_i < l; tmp_sum_i++) \
                          tmp_sum_s += arr[tmp_sum_i]; tmp_sum_s; })
#define SWAP(x, y) ({ __typeof__(x) tmp_swap_z = x; x = y; y = tmp_swap_z; })
#define SPAN_ID(l, b, e) ((int) ((b) + (2 * (l) + 2 - (e) \
                              + (b)) * ((e) - (b) - 1) / 2))
#define STR_CLONE(d, s) ({ d = (char*) malloc((strlen(s) + 1) * sizeof(char)); \
                           strcpy(d, s);})
#define LOG(dbg_level, ...) ({ if(s3eDbgMode >= dbg_level) { \
                                  printf(__VA_ARGS__); fflush(stdout);} })
#define RAND(s) ({ s = ((s * 0x5DEECE66DL + 0x0A) & 0xFFFFFFFFFFFFL); \
                   ((real) s / ((real) 0xFFFFFFFFFFFFL)); })
#define SQ(x) ((x) * (x))
#define RAND_PAIR(x, y, s, l) ({x = RAND(s) * l; y = RAND(s) * (l-1); \
                  if(y >= x) y++;})
...
real CKYDecodeEvalOneSentence(int *word_ids, int word_num, real dc,
  ...
  real v[s3eMaxDimension], w[s3eMaxDimension],
       aw1[s3eMaxDimension], aw2[s3eMaxDimension];
  real score;
...
void CKYDecodeAccGradOneSentence(int *word_ids, int word_num, real gs, real lr,
  ...
  while(bkkp->hd != bkkp->tl) {
    ...
    if(cl != -1) { // composition
      ...
      CompositionAccGrad(
        ...
        bkkp->w + cl * s3eN, bkkp->w + cr * s3eN,
        bkkp->v + c * s3eM, bkkp->w + c * s3eN,
        bkkp->aw1 + c * s3eN, bkkp->aw2 + c * s3eN, bkkp->s[c],
        bkkp->gv + bkkp->tl*s3eM, bkkp->gw + bkkp->tl*s3eN, bkkp->gs[bkkp->tl],
        gd->gm,
        bkkp->gv + hl * s3eM, bkkp->gv + hr * s3eM,
        bkkp->gw + hl * s3eN, bkkp->gw + hr * s3eN);
      bkkp->gs[hl] = dc * bkkp->gs[bkkp->tl];
      bkkp->gs[hr] = dc * bkkp->gs[bkkp->tl];
    }
    else { // lookuptable
      gd->idx[gd->idx_size] = cr;
      LookupTableAccGrad(gd->idx[gd->idx_size],
        bkkp->v + c * s3eM, bkkp->w + c * s3eN,
  ...
  BatchUpdateParam(gd->gm, bgd->gm, VAR, lr, s3eL2PenaltyWeight,
                   gd->idx, bgd->idx, &(gd->idx_size), &(bgd->idx_size));
  // LOG(2, "%s\n", ToStringParam(bgd->gm));
  // LOG(2, "%s\n", ToStringParam(gd->gm));
  if((++(bgd->cnt)) == s3eBsz) {
    BatchUpdateParam(bgd->gm, MDL, 0L, -1.0, 0.0,
                     bgd->idx, 0L, &(bgd->idx_size), 0L);
    ZeroGrad(bgd);
  }
  return;
}
...
void CKYTrainOneSentence(int *words, int *words2, int wn, real lr, real dc,
    ...
    struct Heap *heap, struct Heap *heap2, struct Grad *gd, struct Grad *bgd,
...
void* ThreadedTrain(void *arg) {
  ...
  long long int thread_id, seed, fbeg, fend, fpos,
                int_s_num, int_w_num, int_correct_num, int_wrong_num;
  thread_id = (long) arg;
  ...
  for(iter = 0; iter < s3eMaxIterNum; iter++) {
    ...
    while(1) {
      ...
      if(wn >= 2)
        CKYTrainOneSentence(words, words2, wn, lr, dc, bkkp, bkkp2,
           heap, heap2, gd, bgd, &int_correct_num, &int_wrong_num);
      int_s_num++;
      int_w_num += wn;
      if(int_s_num == 100) {
        prd_sent_num += int_s_num;
        prd_word_num += int_w_num;
        int_accuracy = (int_correct_num + 1e-6) / (int_correct_num
                        + int_wrong_num + 1e-6);
        int_s_num = int_w_num = int_correct_num = int_wrong_num = 0;
        t1 = clock();
        fpos = ftell(fin);
        ...
        if(save_thread_cnt[thread_id] > 0) {
          if(progress >= save_thread_start[thread_id]) {
            ...
            SaveModel((int) (save_thread_start[thread_id] /
                    ((real)(save_thread_interval/s3eThreadNum)) + 1e-6) );
        ...
        //   );
        LOG(2, "[%2lld@%3d:%5.2lf%%:%8.6lf]: %s A %5.3lf \r",
          thread_id, iter, progress, lr, ToStringParam(MDL), accuracy);
      }
      if(t_sent_cnt[thread_id] % 100000 == 0) {
        LOG(2,
          "%s[%2lld@%3d:%5.2lf%%:%8.6lf]: %s A %5.3lf %s \n",
...
int GetOptPos(char *str, int argc, char **argv) {
  int i;
  for (i = 1; i < argc; i++) if(strcmp(str, argv[i]) == 0) return i;
  return -1;
}
